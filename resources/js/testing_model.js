...
ipcRenderer.on('available-to-test', (event, index, id, available) => {
  //console.log('testing_model.js available-to-test');
  RecordingsModel.available = available;
  dataRecord.index = index;
  dataRecord.id = id;
  if(RecordingsModel.isTesting == true && RecordingsModel.available == true){
    console.log('Puede empezar a hacer prueba...');
    startRecording();
  }
     
});

ipcRenderer.on('testing-process',(event, label) => {

  RecordingsModel.labelTest = label;
});

//Starting a new Test

const startTesting = function(){
  RecordingsModel.isTesting = true;
  ipcRenderer.send('start-testing',path.join(RecordingsModel.workspacePath,RecordingsModel.testName));

};

const stopTesting = function(){
  RecordingsModel.isTesting = false;
  ipcRenderer.send('stop-testing');
};
